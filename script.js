...
const nameEl = document.querySelector(
    '.form-container .form-row input[name="name"]'
);
const emailEl = document.querySelector(
    '.form-container .form-row input[name="email"]'
);
...
function createGearsTimelines() {
    ...
    data.forEach((d, dIdx) => {
        ...
        if (gear.hasHole) {
            ...
            pathD += `A ${holeRadius} ${holeRadius} 1 1 0 ${gear.center.x + holeRadius
                } ${gear.center.y}`;
            pathD += `A ${holeRadius} ${holeRadius} 1 1 0 ${gear.center.x - holeRadius
                } ${gear.center.y}`;
        }
            ...
            tl.eventCallback("onUpdate", () => {
                ...
                if (deltaY > 8) {
                    ...
                    if (!state.sumbitBtnOnPlace) {
                        state.sumbitBtnTextOpacity =
                            (sprayRepeatCounter - 1) * 0.3 + 0.3 * textProgress;
                        state.sumbitBtnTextOpacity = Math.pow(
                            state.sumbitBtnTextOpacity,
                            2
                        );
                    }
